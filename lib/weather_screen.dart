...
//               if (!isUsingCurrentLocation)
//                 IconButton(
//                   icon: Icon(isFav ? Icons.star : Icons.star_border, color: isFav ? Colors.yellow : Colors.white),
//                   onPressed: () async {
//                     await _toggleFavorite(w.city);
//                     final msg = favoriteCities.contains(w.city) ? "Added to favorites" : "Removed from favorites";
//                     ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("${w.city} — $msg")));
//                   },
//                 ),
//             ],
//           ),
...
//           Column(
//             children: [
//               Icon(_getIconForWeatherCode(w.weatherCode), size: 84, color: Colors.yellow.shade300),
//               const SizedBox(height: 8),
//               Text(getWeatherDescription(w.weatherCode), style: const TextStyle(fontSize: 18)),
//             ],
//           ),
//           const SizedBox(height: 12),
//           Text(_tempString(w.temperature, precision: 1), style: const TextStyle(fontSize: 56, fontWeight: FontWeight.bold)),
//           const SizedBox(height: 10),
//           Container(
//             margin: const EdgeInsets.symmetric(horizontal: 6),
//             padding: const EdgeInsets.all(12),
...
//               mainAxisAlignment: MainAxisAlignment.spaceAround,
//               children: [
//                 Row(children: [const Icon(Icons.air, size: 26), const SizedBox(width: 6), Text("${w.wind.toStringAsFixed(0)} km/h", style: const TextStyle(fontSize: 16))]),
//                 Row(children: [const Icon(Icons.water_drop, size: 26), const SizedBox(width: 6), Text("${w.humidity ?? '--'}%", style: const TextStyle(fontSize: 16))]),
//               ],
//             ),
//           ),
//           const SizedBox(height: 14),
//           Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
//             Row(children: [const Icon(Icons.wb_sunny_outlined), const SizedBox(width: 6), Text(w.sunrise != null ? _timeFmt.format(w.sunrise!.toLocal()) : '--', style: const TextStyle(fontSize: 14))]),
//             Row(children: [const Icon(Icons.nights_stay), const SizedBox(width: 6), Text(w.sunset != null ? _timeFmt.format(w.sunset!.toLocal()) : '--', style: const TextStyle(fontSize: 14))]),
//           ])
//         ],
//       ),
//     );
...
//             margin: const EdgeInsets.symmetric(vertical: 6),
//             child: ListTile(
//               leading: Icon(_getIconForWeatherCode(day.weatherCode), color: Colors.white),
//               title: Text(_dayFmt.format(day.date), style: const TextStyle(color: Colors.white)),
//               subtitle: Text("Min ${_displayTemp(day.minTemp).toStringAsFixed(0)}° • Max ${_displayTemp(day.maxTemp).toStringAsFixed(0)}°", style: const TextStyle(color: Colors.white70)),
//               trailing: Text("${_displayTemp(mid).toStringAsFixed(0)}°", style: const TextStyle(color: Colors.white)),
//             ),
//           );
//         },
//       ),
...
//     final int weatherCode = current?["weathercode"] is num ? (current["weathercode"] as num).toInt() : 0;
...
//   static List<DailyForecast> fromJsonList(Map<String, dynamic> json) {
//     try {
//       final dates = (json["daily"]["time"] as List<dynamic>).map((e) => DateTime.parse(e)).toList();
//       final maxList = (json["daily"]["temperature_2m_max"] as List<dynamic>).map((e) => (e as num).toDouble()).toList();
//       final minList = (json["daily"]["temperature_2m_min"] as List<dynamic>).map((e) => (e as num).toDouble()).toList();
//       final codeList = (json["daily"]["weathercode"] as List<dynamic>).map((e) => (e as num).toInt()).toList();
